...
var ChilliBaseTemplate = Class({
    ...
    constructor: function (layout, file, file_path) {
		...
		// ============================
		// LEGAL TEXT COLOR OVERRIDE
		// ============================

		try {

			var pin = (this.layout.pinlines || "").toUpperCase();
			var type_line = (this.layout.type_line || "").toLowerCase();

			var is_land = type_line.indexOf("land") !== -1;
			var is_artifact = type_line.indexOf("artifact") !== -1;

			var legal_color = rgb_white(); // default

			if (is_land || is_artifact) {
				legal_color = rgb_white();
			}
			else if (pin === "W" || pin === "R") {
				legal_color = rgb_black();
			}
			else {
				legal_color = rgb_white();
			}

			// aplicar directamente al layer LEGAL
			var legal_layer = this.legal.layers.getByName(LayerNames.LEGAL);
			legal_layer.textItem.color = legal_color;

		} catch (err) {
			alert("❌ ERROR aplicando color legal: " + err);
		}
...
var OldClassicTemplate = Class({
    ...
    constructor: function (layout, file, file_path) {
        ...
        var ft_lead = Number(this.layout.text_flavor_lead);

// Mostrar alert para verificar
//alert("ft_lead = " + ft_lead);
    ...
    // Ejecuta el pipeline normal primero
		...
		try {
			...
			if (this.layout.text_justification !== null) {
				var j = this.layout.text_justification;
