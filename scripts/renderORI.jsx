...
function render(file) {
    ...
    if (in_array(BasicLandNames, card_name)) {
        ...
        // 1️⃣ Llamar a get_card_info.py y obtener JSON
        var scryfall = call_python(card_name, file_path);

        // -------------------- NUEVO --------------------
        // 2️⃣ Llamar a get_set_info.py para agregar setTotalCount
        ...
        // 3️⃣ Volver a leer card.json para incluir setTotalCount
        var json_file = new File(file_path + json_file_path);
        json_file.open('r');
        var json_string = json_file.read();
        json_file.close();
        scryfall = JSON.parse(JSON.parse(json_string));
		//alert("DEBUG set_total_count = " + scryfall.set_total_count);
    ...
    // select and execute the template - insert text fields, set visibility of layers, etc. - y save to disk
