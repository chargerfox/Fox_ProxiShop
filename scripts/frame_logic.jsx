...
function select_frame_layers(mana_cost, type_line, oracle_text, colour_identity_array, colour_indicator) {
    ...
    if (type_line.indexOf(LayerNames.LAND) >= 0) {
        ...
        for (var i in rules_lines) {
			...
			// Condición original (Scryfall moderno)
			var is_modern_tap =
				tap_index >= 0 &&
				colon_index >= 0 &&
				tap_index < colon_index &&
				line.toLowerCase().indexOf("add") >= 0;

			// Condición flexible (texto viejo / custom)
			// Si contiene símbolos de mana explícitos
			var has_mana_symbols = false;
			for (var c in colours) {
				if (line.indexOf("{" + colours[c] + "}") >= 0) {
					has_mana_symbols = true;
					break;
				}
			}

			// Si cumple cualquiera de las dos condiciones
			if (is_modern_tap || has_mana_symbols) {
				for (var colour in colours) {
					if (
						line.indexOf("{" + colours[colour] + "}") >= 0 &&
						colours_tapped.indexOf(colours[colour]) < 0
					) {
