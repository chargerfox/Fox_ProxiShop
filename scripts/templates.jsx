...
var BaseTemplate = Class({
    constructor: function (layout, file, file_path) {
		...
		// luego agregamos el TextField del legal usando push
		try {
			var legal_layer = this.legal.layers.getByName(LayerNames.LEGAL);
			var current_text = legal_layer.textItem.contents; // texto existente en PSD
			legal_layer.textItem.contents = current_text + " " + get_legal_text(this.layout);
			
			this.text_layers.push(
				new TextField(
					layer = legal_layer,
					text_contents = legal_layer.textItem.contents,
					text_colour = rgb_white(),   // 👈 ACA
				)
			);
		} catch (err) {
			$.writeln("Capa LEGAL no encontrada en este PSD, se omite.");
		}
    ...
    basic_text_layers: function (text_and_icons) {
        ...
        this.text_layers = this.text_layers.concat([
            ...
            new ExpansionSymbolField(
                ...
                text_contents = get_expansion_symbol(this.layout), //new
                rarity = this.layout.rarity,
				layout = this.layout,   // 👈 PASAMOS EL LAYOUT
            ),
			//TYPELINE
            new ScaledTextField(
...
var OldClassicTemplate = Class({
    /**
     * A template for 1th Edition frame. 
     */
    ...
    constructor: function (layout, file, file_path) {
        ...
        // ✅ Override ARTIST TextField correctamente
        var artistText = "Illus. \u00A9 " + this.layout.artist;
        try {
            if (this.text_layers && this.text_layers.length > 0) {
                this.text_layers[0].text_contents = artistText;
            }
        } catch (err) {
            alert("❌ Error override artist textfield: " + err);
        }
        ...
        // rules text
        var is_centred = this.layout.flavour_text.length <= 1 &&
                         this.layout.oracle_text.length <= 70 &&
        ...
        $.global.ft_lead_global = this.layout.text_flavor_lead;

		this.text_layers.push(
			new FormattedTextArea(
				rules_text,
				this.layout.oracle_text,
				get_text_layer_colour(rules_text),
				this.layout.flavour_text,
				is_centred,
				reference_layer,
				this.layout.text_size,		// 👈 font que viene del set.js
				this.layout.text_leading	//igual
			)
		);
    ...
    // =====================================
    // 🔥 FORCE HIDE LAYERS AFTER RENDER
    // =====================================
    execute: function () {

    // Ejecuta el pipeline normal primero
		var file_name = this.super();

		try {
			var docref = app.activeDocument;

			// Solo ocultar el layer de texto "Legal" dentro del grupo Legal
			var legal_group = docref.layers.getByName(LayerNames.LEGAL);
			var legal_text = legal_group.layers.getByName(LayerNames.LEGAL); // el text dentro
			legal_text.visible = false;

			// Ocultar Expansion Symbol
			var text_and_icons = docref.layers.getByName(LayerNames.TEXT_AND_ICONS);
			var expansion = text_and_icons.layers.getByName(LayerNames.EXPANSION_SYMBOL);
			expansion.visible = false;
			
			// ⚡ Ajuste de tamaño del Mana Cost SOLO DESPUÉS de renderizar
			var mana_cost = text_and_icons.layers.getByName(LayerNames.MANA_COST);
			mana_cost.textItem.size = 11.2; // ahora sí aplica	
			
			var Ref_text = text_and_icons.layers.getByName("Textbox Reference[defunct]");
			var Ref_text2 = text_and_icons.layers.getByName("Textbox Reference");
			Ref_text.visible = false; // lo ocultás
			Ref_text2.visible = false;
				
			//**********************************	
			// justified, size, tracking, leading, etc
			//**********************************
			var rules_text = text_and_icons.layers.getByName(LayerNames.RULES_TEXT);
			var ti = rules_text.textItem;

			// ---- SIZE ----
			if (this.layout.text_size !== null) {
				var new_size = Number(this.layout.text_size);
				if (!isNaN(new_size) && new_size > 0) {
					ti.size = new_size;
					$.writeln("✔ Rules Text size aplicado: " + new_size);
				}
			}

			var j = String(this.layout.text_justification).toUpperCase();

			// ---- JUSTIFICATION ----
			if (this.layout.text_justification !== null) {
				...
				switch (j) {
					...
					default:
						$.writeln("⚠ Justification desconocido: " + j);
				}

				$.writeln("✔ Rules Text justification aplicado: " + j);
			}


			// ---- TRACKING ----
			if (this.layout.text_tracking !== null) {
				var new_tracking = Number(this.layout.text_tracking);
				if (!isNaN(new_tracking)) {
					ti.tracking = new_tracking;
					$.writeln("✔ Rules Text tracking aplicado: " + new_tracking);
				}
			}

			/*// ---- LEADING ----
			if (this.layout.text_leading !== null) {
				var new_leading = Number(this.layout.text_leading);
				if (!isNaN(new_leading) && new_leading > 0) {
					ti.leading = new_leading;
					$.writeln("✔ Rules Text leading aplicado: " + new_leading);
				}
			}*/

			// ---- OFFSET X ----
			if (this.layout.offsetx !== null) {
				var new_x = Number(this.layout.offsetx);
				if (!isNaN(new_x)) {
					var pos = ti.position;
					ti.position = [ pos[0] + new_x, pos[1] ];
					$.writeln("✔ Rules Text offset X aplicado: " + new_x);
				}
			}

			// ---- OFFSET Y ----
			if (this.layout.offsety !== null) {
				var new_y = Number(this.layout.offsety);
				if (!isNaN(new_y)) {
					var pos = ti.position;
					ti.position = [ pos[0], pos[1] + new_y ];
					$.writeln("✔ Rules Text offset Y aplicado: " + new_y);
				}
			}

			// ---- BOX WIDTH ADJUST ----
			if (this.layout.boxwidthadjust !== null) {
				var new_w = Number(this.layout.boxwidthadjust);
				if (!isNaN(new_w)) {
					var b = rules_text.bounds; 
					b[2] = b[2] + new_w;
					rules_text.bounds = b;
					$.writeln("✔ Rules Text box width ajustado: " + new_w);
				}
			}
		
		} catch (err) {
			$.writeln("⚠️ Error cambiando datos post render: " + err);
		}
...
var RegularTemplate = Class({
    ...
    // Ejecuta el pipeline normal primero
		...
		// COLOR DEL TEXTO ARTIST Y LEGAL SEGUN CARTA
		try {
			var docref = app.activeDocument;
			var legal_group = docref.layers.getByName(LayerNames.LEGAL);

			if (!legal_group) return file_name;

			var artist_layer = legal_group.layers.getByName(LayerNames.ARTIST);
			var legal_layer  = legal_group.layers.getByName(LayerNames.LEGAL);

			// ===============================
			// COLOR LOGIC (USANDO TYPES)
			// ===============================
			var makeWhite = false;

			// types siempre es array: ["Artifact", "Creature", ...]
			var types = this.layout.types || [];
			var pin   = (this.layout.pinlines || "").toUpperCase();

			if (in_array(types, "Land") || pin === "B") {
				makeWhite = true;
			}
			...
			if (artist_layer && artist_layer.kind === LayerKind.TEXT) {
				artist_layer.textItem.color = color;
			}

			if (legal_layer && legal_layer.kind === LayerKind.TEXT) {
				legal_layer.textItem.color = color;
			}

			$.writeln("✔ Legal/Artist color aplicado: " + (makeWhite ? "BLANCO" : "NEGRO"));

		} catch (err) {
			$.writeln("⚠ Error aplicando color Legal/Artist: " + err);
		}
    ...
    enable_frame_layers: function () {
		...
		// --- Paso 1: activar el frame normal ---
		var layers = docref.layers.getByName(LayerNames.NONLAND);
		var selected_layer = this.layout.background;
		if (this.is_land) {
			layers = docref.layers.getByName(LayerNames.LAND);
		...
		try {
			...
			$.writeln("⚠️ enable_frame_layers: layer '" + selected_layer + "' no encontrado en frame folder");
		}

		// -----------------------------
		// MOSTRAR PT FRAME (solo criaturas)
		// -----------------------------
		try {
			var docref = app.activeDocument;
			var pt_folder = docref.layers.getByName("PT");

			// ocultar todos los PT
			for (var i = 0; i < pt_folder.layers.length; i++) {
				pt_folder.layers[i].visible = false;
			}

			// 🔑 LEER TYPES DESDE LAYOUT
			var types = (this.layout.types && this.layout.types.constructor === Array)
				? this.layout.types
				: [];

			var pin = (this.layout.pinlines || "").toUpperCase();

			// DEBUG FINAL (dejalo hasta que confirmes)
			alert(
				"PT DEBUG\n\n" +
			...
			// ⛔ si no es criatura, no hay PT
			if (types.indexOf("Creature") === -1) {
				alert("NO es Creature → sin PT");
				return;
			}
			...
			// PRIORIDAD
			if (types.indexOf("Artifact") !== -1) {
				pt_layer_name = "Artifact";
			}
			else if (pin.indexOf("/") !== -1 || pin === "GOLD") {
				pt_layer_name = "Gold";
			}
			else {
				pt_layer_name = pin; // W U B R G
			}


			var pt_layer = pt_folder.layers.getByName(pt_layer_name);
			pt_layer.visible = true;

			alert("PT ACTIVADO: " + pt_layer_name);
