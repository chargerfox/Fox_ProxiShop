...
var BaseLayout = Class({
    ...
    unpack_scryfall: function () {
         ...
         */
		this.set = this.scryfall.set;   // 👈 IMPORTANTE
		this.collector_number = this.scryfall.collector_number;   // nuevo
		this.set_total_count = this.scryfall.set_total_count || "";   // ✅ FIX 
		this.text_style = this.scryfall.text_style || {};   // ✅ SIEMPRE OBJETO
		this.types = this.scryfall.types || []; //Typo de carta. Creature,Artifact,Enchantment, etc.
		// -----------------------------
		// Text style normalization
		// -----------------------------

		this.text_justification = (
			this.text_style.justification !== undefined
		) ? this.text_style.justification : null;

		this.text_size = (
			this.text_style.size !== undefined
		) ? this.text_style.size : null;

		this.text_tracking = (
			this.text_style.tracking !== undefined
		) ? this.text_style.tracking : null;

		this.text_leading = (
			this.text_style.leading !== undefined
		) ? this.text_style.leading : null;

		this.text_flavor_lead = (
			this.text_style.text_flavor_lead !== undefined
		) ? this.text_style.text_flavor_lead : null;

		// ⚠️ NOMBRES CORREGIDOS (sin _)
		this.offsetx = (
			this.text_style.offsetx !== undefined
		) ? this.text_style.offsetx : null;

		this.offsety = (
			this.text_style.offsety !== undefined
		) ? this.text_style.offsety : null;

		this.boxwidthadjust = (
			this.text_style.boxwidthadjust !== undefined
		) ? this.text_style.boxwidthadjust : null;
